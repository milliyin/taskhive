...
export default function WithdrawClaimButton({ claimId }: { claimId: number }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="rounded-full bg-md-error-container px-2 py-1 text-xs font-medium text-md-error transition-all duration-200 hover:bg-md-error/10 active:scale-95 disabled:opacity-50"
    >
      {loading ? "..." : "Withdraw"}
    </button>
  );
